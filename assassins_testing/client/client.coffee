...
Template.adminContent.helpers
  ...
  deadUsers: ->
    Meteor.users.find({alive: false, isAdmin: false}, {sort: {index: 'asc'}}).fetch()
...
Template.gameContent.helpers
  leaderBoard: ->
    Meteor.users.find({isAdmin: false}, {sort: {kills: 'asc'}}).fetch().slice(0, 5)

  firstBloodUsers: ->
    Meteor.users.find({firstBlood: true}).fetch()
